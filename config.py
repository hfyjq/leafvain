...
if not WORKSPACE_DIR.exists():
    ...
    print(f"✅ 创建工作区目录: {WORKSPACE_DIR}")

# 打印验证信息（调试用）
print(f"⭐ 配置文件位置: {__file__}")
...
"""
SHORT_TERM_MEMORY_SIZE = 5  # 保留最近5轮对话
MEDIUM_TERM_MEMORY_DIR = "workspace/memory/medium_term"
LONG_TERM_VECTOR_DB = "workspace/memory/vector_db"
MEMORY_COMPRESSION_PROMPT = "请用简洁的语言总结以下对话的核心信息，保留关键事实和承诺："
# 最大执行步骤
...
# API配置
ZHIPU_API_KEY = os.getenv("ZHIPU_API_KEY", "your_actual_api_key_here")
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "your_actual_api_key_here")
ANTHROPIC_API_KEY = os.getenv("ANTHROPIC_API_KEY", "your_actual_api_key_here")

# 默认使用的API提供商
DEFAULT_API_PROVIDER = "zhipu"  # 可选: "zhipu", "openai", "anthropic", "test"
